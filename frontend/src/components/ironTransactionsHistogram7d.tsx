...
export function IronTransactionsHistogram7d() {
  const { data, loading, error } = useIronPeriodSummary("7d")

  const dailyData =
    data?.daily_data?.map((item) => {
      // item.date now contains the week label (e.g., "Nov 20-26")
      ...
      return {
        ...
        onramps: item.total_onramps,
        offramps: item.total_offramps,
              ...
              <Tooltip
                ...
                labelFormatter={(label, payload) => {
                  if (payload && payload[0]?.payload?.dateFull) {
                    ...
                    const weekRange = payload[0].payload.dateFull // Already formatted as "Nov 20-26"
                    return isOngoing ? `${weekRange} (Ongoing)` : weekRange
                  }
                  return label
                }}
                formatter={(val: number, name: string) => {
                  const label = name === "totalTransactions" ? "Total" : name === "onramps" ? "Onramps" : "Offramps"
                  return [val, label]
                }}
              />
              <Bar
